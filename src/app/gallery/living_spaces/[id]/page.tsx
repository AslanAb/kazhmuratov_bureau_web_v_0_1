...
export default function LivingSpaceById({
  params
}: {
  params: { id: string }
}) {
  const data = living_spaces_projects[+params.id - 1]
        ...
        <div className="grid grid-cols-3 gap-5 mb-16">
          {data.src_of_images.map((item) => {
            return (
              <div
                key={item.img_id}
                className="relative w-full h-[35vh] hover:scale-110 transition duration-200 ease-in-out delay-150"
                onClick={() => {
                  setShowModal(true)
                  setImg(item)
                }}
